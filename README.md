...
|       schema\_meta       | string |  false   |           |                   Single schema analysis file path<br>to SchemaMeta\_xml, when running `_all`<br>or `_schemas` path to directory<br>containing SchemaMeta\_xmls with pattern (DatabaseName|Schema).meta.xml<br>                    |
